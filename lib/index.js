...
const DEFAULT_METRICS_PAGESUMMARY = [
  ...
  'score',
  'pageSummary.lighthouseResult.categories.performance.score',
  'pageSummary.score'
];
    ...
    switch (message.type) {
      ...
      case 'url': {
        ...
        return analyzer.analyzeUrl(url, log, this.options).then(result => {
          ...
          log.info('Result from Google Page Speed Insights:%:2j', result);
          result.score = (result.lighthouseResult.categories.performance.score || 0) * 100;
          queue.postMessage(
